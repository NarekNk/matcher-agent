...
# Optional: when ``--track-language`` is omitted, ``recommend`` runs local Whisper
# (``openai-whisper``, bundled in ``pyproject.toml``) on the 30s preview to detect
# language — no API key. Tune model size vs. accuracy/latency:
WHISPER_LANGUAGE_DETECTION=1
# WHISPER_MODEL=tiny
# WHISPER_DEVICE=   # empty = cuda if available else cpu; or force ``cuda`` / ``cpu``
...
## Supabase artifact upload CLI

After you sync/build/train locally, upload only the inference artifacts:

```bash
PYTHONPATH=src music-env/bin/python -m matcher_agent.cli.upload_supabase \
...
- `--root-dir .` to point at a different project root.
- `--exclude-embeddings` to skip `data/embeddings/text_embeddings.parquet`.
- `--dry-run` to print the manifest without uploading.

Default uploaded keys:
...
- `data/embeddings/text_embeddings.parquet` (unless excluded)

---

## Standalone Railway FastAPI app

A standalone deployment unit lives in `deploy/railway_fastapi_app`.

It downloads runtime files from Supabase on startup and exposes:

- `GET /health`
- `GET /recommend` (same query contract as `matcher_agent.api.recommend_api`)

See `deploy/railway_fastapi_app/README.md` for required env vars, bucket layout, and Docker build notes.
